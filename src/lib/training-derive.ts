...
export function toWeeklyBuckets(
  ...
  for (let cursor = firstWeek; cursor.getTime() <= lastWeek.getTime(); cursor = addDays(cursor, 7)) {
    ...
    buckets.set(key, { weekStart: key, swim: 0, bike: 0, run: 0 });
  }
  ...
  for (const activity of activities) {
    ...
    bucket[activity.discipline] += activity.movingSeconds;
  }
...
export function toDailyCells(
  ...
  for (let cursor = firstDay; cursor.getTime() <= lastDay.getTime(); cursor = addDays(cursor, 1)) {
    const key = cursor.toISOString().slice(0, 10);
    cells.set(key, { date: key, swim: 0, bike: 0, run: 0, total: 0 });
  }
  ...
  for (const activity of activities) {
    ...
    if (!cell) continue;
    cell[activity.discipline] += activity.movingSeconds;
    cell.total += activity.movingSeconds;
  }
